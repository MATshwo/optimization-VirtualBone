...
def Training(config_path,out_path,cluster_id=0,device="cpu",mode="LF",queue=None):
    
    # 设置cuda设备
    pre_device = "cuda:1"  #预训练模型加载设备
    ...
    if mode == 'LF':
        ...
        # if os.path.exists(os.path.join(detail_net_path_save,"spectral/Cluster_cos_{}_0412.pth.tar".format(cluster_id))):
        #    # 如果存在,就在原始模型上继续训练
        #    detail_model.load_state_dict(torch.load(os.path.join(detail_net_path_save,"spectral/Cluster_cos_{}_0412.pth.tar".format(cluster_id)))) 
        ...
        detail_model.train()
        groundHF = DataLoader(dataset=HFdata,batch_size=Batchsize_hf,shuffle=True,drop_last=True,num_workers=1)
        loss_list = []
...
if __name__ == "__main__":
    ...
    for i in range(9):
        ...
        p = Process(target=Training,args=(config_path,out_path,i,device,"HF",queue))
        p.start()
        processes.append(p)
